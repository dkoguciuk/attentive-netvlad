...
def train_one_epoch(sess, ops, train_writer, test_writer, epoch, saver):
    ...
    for i in range(len(train_file_idxs)//BATCH_NUM_QUERIES):
        ...
        # if True:
            ...
            # import pickle
            # with open('TRAINING_LATENT_VECTORS.pkl', 'wb') as f:
            #     pickle.dump(TRAINING_LATENT_VECTORS, f, protocol=4)
            print("Calculated cached feature vectors")

            # import pickle
            # with open('TRAINING_LATENT_VECTORS.pkl', 'rb') as f:
            #      TRAINING_LATENT_VECTORS = pickle.load(f)
            # print("Load cached feature vectors")
            # #exit()
